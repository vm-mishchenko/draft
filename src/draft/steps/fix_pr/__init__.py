...
def _build_prompt(ctx, verify_commands: str, failed_checks: list[dict]) -> str:
    template = files("draft.steps.fix_pr").joinpath("fix_pr.md").read_text()
    ...
    if spec_path:
        ...
        if path.is_file():
            try:
                ...
                spec = ""
    verify_errors = ctx.step_get("fix-pr", "verify_errors", "")
    if verify_errors:
...
def _run_git_capture(cmd: list[str], cwd: str, timeout: float, log_path: Path) -> str:
    try:
        ...
        raise StepError("fix-pr", TIMEOUT_EXIT) from None
    ...
    if result.returncode != 0:
        raise StepError("fix-pr", result.returncode)
    return stdout
...
def _generate_commit_message(
    ...
    template = files("draft.steps.fix_pr").joinpath("commit_message.md").read_text()
    ...
    print(
        f"fix-pr: commit-message agent failed {max_attempts} times; using fallback '{fallback}'",
        file=sys.stderr,
    )
...
class FixPrStep(Step):
    ...
    def run(self, ctx, engine, lifecycle, step_metrics):
        ...
        with engine.stage(self.name) as s:
            commit_msg_log = ctx.run_dir / "fix-pr-commit-msg.log"
            verify_commands = _render_verify_commands(
                lifecycle.get_hooks(self.name, "verify")
            )

            counts, entries = _check_ci(pr_url)
            failed_checks = [e for e in entries if e["state"] == "failure"]
            if not failed_checks:
                ctx.step_set(
                    self.name,
                    "verify_errors",
                    "no failing checks at step start; nothing to fix",
                )
                ctx.save()
            ...
            for attempt in range(1, cfg["max_retries"] + 1):
                ...
                engine.run_llm(
                    prompt=_build_prompt(ctx, verify_commands, failed_checks),
                    cwd=wt_dir,
                ...
                if not _has_changes(wt_dir):
                    ctx.step_set(
                        ...
                        "instruction (the fix-pr prompt forbids commits)",
                    )
                ...
                shortstat = _run_git_capture(
                    ["git", "diff", "--shortstat", "HEAD~1"], wt_dir, 30, commit_msg_log
                ).strip()
